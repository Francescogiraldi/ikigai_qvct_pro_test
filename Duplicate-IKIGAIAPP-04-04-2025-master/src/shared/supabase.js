...
const options = {
  ...
  global: {
    ...
    fetch: (url, options) => {
      ...
      .catch(error => {
        console.error("Erreur de fetch Supabase:", error.name); // Modifié pour afficher error.name
        // Retourner une erreur formatée
        if (error.name === 'AbortError') {
          throw new Error('La requête a pris trop de temps à s\'exécuter. Veuillez vérifier votre connexion internet.'); // Modifié le message d'erreur
        }
        throw error;
      });
    }
